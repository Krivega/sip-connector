...
- Событийная система с префиксами
...
**Принцип работы**: Все операции ConnectionManager выполняются через очередь для предотвращения гонки условий.

---

### 5. **CallManager** (WebRTC-звонки)
...
### 7. **PresentationManager** (Презентации)
...
### 8. **IncomingCallManager** (Входящие звонки)
...
### 9. **TransceiverManager** (Управление transceiver'ами)

**Назначение**: Отслеживание и классификация RTCRtpTransceiver'ов.

**Ключевые возможности**:

- Автоматическая классификация по `mid` значению
...
- `mainAudio` (mid='0') - основной аудио поток
...
- `AbstractCallStrategy` - базовая реализация
...
graph TB
    subgraph "SIP-Connector Architecture"
        ...
        subgraph "Core Managers"
            ...
            D["CallManager<br/>📞 WebRTC Calls<br/>+ TransceiverManager"]
            ...
            H["StatsManager<br/>📊 WebRTC Stats"]
            I["VideoSendingBalancerManager<br/>⚖️ Video Optimization"]
...
- `CallManager` → `ICallStrategy` (паттерн Стратегия)
...
- Сбор WebRTC статистики
- Мониторинг качества соединения
- Автоматическая отправка метрик на сервер

#### **VideoSendingBalancerManager** (Балансировка видео)

- Автоматическая оптимизация видеопотоков
...
// - VideoBalancer автоматически оптимизирует качество
```
...
- **🆕 TransceiverManager**: Автоматическое управление RTCRtpTransceiver'ами
- **🆕 Умные restart события**: Автоматическое добавление презентационных transceiver'ов
- **🆕 Адаптивная балансировка**: Снижение CPU нагрузки на 40-60%
- **🆕 Последовательные операции**: Предотвращение конфликтов соединений

Архитектура обеспечивает **простоту использования**, **высокую производительность** и **легкую расширяемость** для создания качественных видеоконференций.
