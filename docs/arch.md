# Архитектура SIP Connector

Добро пожаловать в документацию по архитектуре SIP Connector. Эта документация разделена на несколько разделов для удобства навигации.

## Структура документации

### 📋 [Обзор архитектуры](./overview.md)

- Архитектурные слои
- Основные компоненты (SipConnectorFacade, SipConnector)
- Диаграмма архитектуры
- Взаимодействие компонентов

### 🧩 [Компоненты](./components.md)

Детальное описание всех менеджеров:

- ConnectionManager (SIP-соединения)
- ConnectionQueueManager (Очередь операций)
- AutoConnectorManager (Автоматическое переподключение)
- CallManager (WebRTC-звонки)
- ConferenceStateManager (Состояние конференции)
- ApiManager (Серверное API)
- PresentationManager (Презентации)
- IncomingCallManager (Входящие звонки)
- StatsManager (Статистика)
- VideoSendingBalancerManager (Балансировка видео)

### 🔄 [Модель состояний (XState)](./state-machines.md)

- Диаграмма состояний сеанса
- Описание всех машин состояний:
  - ConnectionStateMachine
  - CallStateMachine
  - PresentationStateMachine
  - IncomingCallStateMachine
- API для клиентов
- Инварианты и гварды
- Тестирование

### 🏗️ [Архитектурные принципы](./principles.md)

- Разделение ответственности
- Паттерны проектирования
- SOLID принципы
- Ключевые особенности архитектуры
- Преимущества архитектуры

### 💡 [Примеры использования](./examples.md)

- Базовый пример
- Работа с сессией (XState)
- Обработка событий
- Управление медиа-потоками
- Работа с входящими звонками
- Автоматическое переподключение

## Быстрый старт

Для начала работы с SIP Connector рекомендуется:

1. Прочитать [Обзор архитектуры](./overview.md) для понимания общей структуры
2. Изучить [Примеры использования](./examples.md) для практического применения
3. Ознакомиться с [Компонентами](./components.md) для детального понимания работы системы
4. Изучить [Модель состояний](./state-machines.md) для работы с XState

## Основные концепции

SIP Connector построен на принципах **чистой архитектуры** с использованием современных паттернов проектирования:

- **Модульность** - каждый компонент решает свою задачу
- **Расширяемость** - легко добавлять новые функции
- **Надежность** - предсказуемое поведение через машины состояний
- **Производительность** - оптимизация на всех уровнях
- **Простота использования** - высокоуровневый API скрывает сложность

---

**Примечание**: Для получения актуальной информации о конкретных компонентах, обратитесь к соответствующим разделам документации.
