...
Если в конфигурации подключения есть `maxAvailableResolution`, `startPresentation()` и
`updatePresentation()` автоматически добавляют или ужесточают `scaleResolutionDownBy` в
