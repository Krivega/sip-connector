...
- **Команда (отложенная)**: DeferredCommandRunner инкапсулирует отложенное выполнение команды до наступления условия (например, появление токена при переходе CallStateMachine в IN_ROOM); устраняет гонку событий с сервером при переводе участника в зрители
