...
Подробные примеры инициализации, подключения, звонков и управления состоянием доступны в [рецепте "Базовый пример и быстрый старт"](./docs/recipes/basic-setup.md).

---

## 📞 Входящие звонки

Подробные примеры обработки входящих вызовов и управления их состоянием доступны в [рецепте "Работа с входящими звонками"](./docs/recipes/incoming-calls.md).

---

## 🖥️ Управление презентациями

Подробные примеры запуска, настройки качества и управления состоянием презентаций доступны в [рецепте "Управление презентациями"](./docs/recipes/presentations.md).

---

## 👥 Управление участниками конференции

Подробные примеры отслеживания перемещений участников и административных функций доступны в [рецепте "Управление участниками конференции"](./docs/recipes/conference-participants.md).

---

## 📊 Управление медиа-потоками

Подробные примеры работы с удаленными потоками, обработки готовых потоков и управления разрешениями доступны в [рецепте "Управление медиа-потоками"](./docs/recipes/media-management.md).

### Перезапуск ICE-соединения

Подробные примеры ручного и автоматического перезапуска ICE доступны в [рецепте "Перезапуск ICE-соединения"](./docs/recipes/ice-restart.md).
...
Подробные примеры обработки событий, включая основные события, продвинутые паттерны и события балансировки видео, доступны в [рецепте "Обработка событий"](./docs/recipes/events-handling.md).

---

## 📈 WebRTC Статистика

SDK предоставляет детальную WebRTC-статистику, основанную на [W3C WebRTC Statistics API](https://www.w3.org/TR/webrtc-stats/), включающую RTP статистику, кодеки, ICE кандидаты и транспортную информацию.

Подробные примеры использования статистики, адаптивного интервала сбора и типов статистики доступны в [рецепте "WebRTC Статистика"](./docs/recipes/webrtc-stats.md).

---

## ⚡ Адаптивное опрашивание видеопотоков

SDK использует **адаптивное опрашивание** для мониторинга изменений в видеопотоках, что значительно снижает нагрузку на CPU на 40-60% при стабильном видео.

Подробные примеры использования, алгоритм адаптации и настройки доступны в [рецепте "Адаптивное опрашивание видеопотоков"](./docs/recipes/adaptive-polling.md).

---

## 🎛️ Управление видеобалансировщиком

`VideoSendingBalancer` интегрирован в `SipConnector` и запускается автоматически для оптимизации качества видео.

Подробные примеры настройки, жизненного цикла и событий балансировщика доступны в [рецепте "Управление видеобалансировщиком"](./docs/recipes/video-balancing.md).
...
Подробные примеры использования и интеграции доступны в [рецепте "Управление последовательными операциями"](./docs/recipes/connection-queue.md).
...
Подробные примеры настройки, принципа работы и событий автоподключения доступны в [рецепте "Автоматическое переподключение"](./docs/recipes/auto-reconnection.md).
...
Подробные рекомендации по обработке ошибок, восстановлению соединения, управлению ресурсами и оптимизации производительности доступны в [рецепте "Лучшие практики"](./docs/recipes/best-practices.md).

---

## 🐛 Отладка и диагностика

Подробные примеры включения отладочного режима, мониторинга состояния и диагностики доступны в [рецепте "Отладка и диагностика"](./docs/recipes/debugging.md).
