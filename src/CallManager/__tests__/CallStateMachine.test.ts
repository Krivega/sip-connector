...
describe('CallStateMachine', () => {
  ...
  describe('Геттеры состояний', () => {
    ...
    it('isActive должен возвращать true для IN_ROOM, PURGATORY и P2P_ROOM', () => {
  ...
  describe('currentRoom', () => {
    it('возвращает undefined в IDLE', () => {
      expect(machine.currentRoom).toBeUndefined();
    });

    it('возвращает undefined в CONNECTING без room', () => {
      machine.send({ type: 'CALL.CONNECTING', ...connectPayload });

      expect(machine.currentRoom).toBeUndefined();
    });

    it('возвращает room в CONNECTING после ENTER_ROOM', () => {
      machine.send({ type: 'CALL.CONNECTING', ...connectPayload });
      machine.send({ type: 'CALL.ENTER_ROOM', ...room1Payload });

      expect(machine.currentRoom).toBe(room1Payload.room);
    });

    it('возвращает room в PURGATORY', () => {
      machine.send({ type: 'CALL.CONNECTING', ...connectPayload });
      machine.send({ type: 'CALL.ENTER_ROOM', ...purgatoryPayload });

      expect(machine.state).toBe(EState.PURGATORY);
      expect(machine.currentRoom).toBe(PURGATORY_CONFERENCE_NUMBER);
    });

    it('возвращает room в P2P_ROOM', () => {
      machine.send({ type: 'CALL.CONNECTING', ...connectPayload });
      machine.send({ type: 'CALL.ENTER_ROOM', ...p2pRoomPayload });

      expect(machine.state).toBe(EState.P2P_ROOM);
      expect(machine.currentRoom).toBe(p2pRoomPayload.room);
    });

    it('возвращает room в IN_ROOM', () => {
      machine.send({ type: 'CALL.CONNECTING', ...connectPayload });
      machine.send({ type: 'CALL.ENTER_ROOM', ...room1Payload });
      machine.send({ type: 'CALL.TOKEN_ISSUED', token: token1Context.token });
    ...
    it('возвращает обновлённый room при смене room', () => {
      machine.send({ type: 'CALL.CONNECTING', ...connectPayload });
      machine.send({ type: 'CALL.ENTER_ROOM', ...room1Payload });
      ...
      expect(machine.currentRoom).toBe(room2Payload.room);
    });

    it('возвращает undefined после RESET', () => {
      machine.send({ type: 'CALL.CONNECTING', ...connectPayload });
      machine.send({ type: 'CALL.ENTER_ROOM', ...room1Payload });
