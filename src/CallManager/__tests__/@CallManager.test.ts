...
describe('CallManager - дополнительные тесты для покрытия', () => {
  ...
  describe('number, isCallInitiator, currentRoom', () => {
    ...
    it('currentRoom: возвращает undefined без enter-room', () => {
      ...
      expect(cm.currentRoom).toBeUndefined();
    });

    it('currentRoom: возвращает room после enter-room', () => {
      const managers = createManagers();
      const cm = managers.callManager;
      const { apiManager } = managers;

      cm.events.trigger('start-call', { number: '100', answer: false });
      apiManager.events.trigger('enter-room', { room: 'room-1', participantName: 'User' });

      expect(cm.currentRoom).toBe('room-1');

      apiManager.events.trigger('enter-room', { room: 'room-2', participantName: 'User' });

      expect(cm.currentRoom).toBe('room-2');
    });

    it('isEstablishedRTCSession: возвращает false без установленной сессии', () => {
      expect(callManager.isEstablishedRTCSession).toBe(false);
    });

    it('isEstablishedRTCSession: возвращает true при установленной сессии', () => {
      // @ts-expect-error
      // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
      ...
      expect(callManager.isEstablishedRTCSession).toBe(true);
    });
  });
