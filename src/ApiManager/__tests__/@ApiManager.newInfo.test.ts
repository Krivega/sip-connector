...
describe('ApiManager (NEW_INFO handling)', () => {
  ...
  describe('обработка RESTART событий', () => {
    it('должен обрабатывать RESTART события с полными данными', () => {
      const restartSpy = jest.fn();

      apiManager.on('restart', restartSpy);
      mockRequest.setHeader(EHeader.CONTENT_TYPE, EContentTypeReceived.RESTART);
      mockRequest.setHeader(EHeader.TRACKS_DIRECTION, 'incoming');
      mockRequest.setHeader(EHeader.AUDIO_TRACK_COUNT, '2');
      mockRequest.setHeader(EHeader.VIDEO_TRACK_COUNT, '1');
    ...
    it('должен обрабатывать RESTART события с outgoing направлением', () => {
      const restartSpy = jest.fn();

      apiManager.on('restart', restartSpy);
      mockRequest.setHeader(EHeader.CONTENT_TYPE, EContentTypeReceived.RESTART);
      mockRequest.setHeader(EHeader.TRACKS_DIRECTION, 'outgoing');
      mockRequest.setHeader(EHeader.AUDIO_TRACK_COUNT, '1');
      mockRequest.setHeader(EHeader.VIDEO_TRACK_COUNT, '3');
    ...
    it('должен обрабатывать RESTART события с нулевыми значениями треков', () => {
      const restartSpy = jest.fn();

      apiManager.on('restart', restartSpy);
      mockRequest.setHeader(EHeader.CONTENT_TYPE, EContentTypeReceived.RESTART);
      mockRequest.setHeader(EHeader.TRACKS_DIRECTION, 'incoming');
      mockRequest.setHeader(EHeader.AUDIO_TRACK_COUNT, '0');
      mockRequest.setHeader(EHeader.VIDEO_TRACK_COUNT, '0');
    ...
    it('должен обрабатывать RESTART события с большими значениями треков', () => {
      const restartSpy = jest.fn();

      apiManager.on('restart', restartSpy);
      mockRequest.setHeader(EHeader.CONTENT_TYPE, EContentTypeReceived.RESTART);
      mockRequest.setHeader(EHeader.TRACKS_DIRECTION, 'incoming');
      mockRequest.setHeader(EHeader.AUDIO_TRACK_COUNT, '10');
      mockRequest.setHeader(EHeader.VIDEO_TRACK_COUNT, '5');
    ...
    it('должен игнорировать RESTART события не от REMOTE', () => {
      const restartSpy = jest.fn();
