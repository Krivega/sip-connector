...
При создании через `SipConnector` менеджер получает callback `getMaxResolution` — ссылку на
`connectionConfiguration.maxAvailableResolution`. При `startBalancing`, `stopBalancing` и `balance`
контекст `{ getMaxResolution }` передаётся в `VideoSendingBalancer`, чтобы лимит подключения
сохранялся при балансировке, командах MCU и сбросе.
